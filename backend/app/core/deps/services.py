...
def get_openai_client(request: Request) -> AzureOpenAIClient:
    return cast("AzureOpenAIClient", request.app.state.services.openai_client)


def get_rag_service(request: Request) -> RAGService:
    return cast("RAGService", request.app.state.services.rag_service)


def get_storage_service(request: Request) -> BlobStorageService:
    return cast("BlobStorageService", request.app.state.services.storage_service)


def get_search_indexer(request: Request) -> SearchIndexer:
    return cast("SearchIndexer", request.app.state.services.search_indexer)


def get_graph_service(request: Request) -> MicrosoftGraphService:
    return cast("MicrosoftGraphService", request.app.state.services.graph_service)
