...
fallback if nothing matches or anything errors.

Safety guards (callers enforce):
  - Skip routing when a tool flow is in progress (RISK_REGISTER).
  - Skip routing on follow-up messages — only first turn gets classified.
  - A feature-flag killswitch (settings.chat_smart_routing) lets us
